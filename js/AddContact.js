...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    const save = (event) => {
        ...
        const createAndUpdateStorage = () => {
            ...
            if(contactList){
                let contactData=contactList.find(personData => personData._name == contactObj._name);
            if(!contactData){
                    contactList.push(createContactData());
            }else{
                const index=contactList.map(personData => personData._name).indexOf(contactData._name);
